...
<script type="text/javascript">
	...
	//TODO : Ganti Api Key
	const pusher = new Pusher('c5685f1ba6d49d72acdd', {
		cluster: 'ap1'
	});
	const channel = pusher.subscribe('channel-pesanan');
	channel.bind('pesanan-masuk', function (data) {
		$('#notif-count').text(data.count);
		$('#notif-container').prepend(`<div class="dropdown-divider"></div>
          <a  href="<?php echo base_url()?>page/admin/pesanan/lihat?id=${data.pesanan}&token=${data.token}" class="dropdown-item notif-item">
            <i class="fas fa-cart mr-2"></i>
            <span class="float-left text-sm">Pesanan Baru : ${data.tanggal}</span>
          </a>`);
		Toast.fire({
			type: 'info',
			title: 'Ada Pesanan Baru, Silahkan cek klik notifikasi untuk melihat lebih detailnya.'
		})
	});
